...
class Tes_Pengayaan_Waktu_Siswa extends CI_Controller {
	...
	public function Waktu_Mulai_Dan_Durasi_Soal()
	{
		if($this->session->userdata('user') != 'siswa') {
			...
			$format_waktu = strtotime($this->M_Tes_Pengayaan_Waktu_Siswa->Waktu_Mulai($this->input->post('id_tes'))->row()->Waktu_Mulai);
			...
			$durasi = $this->M_Tes_Pengayaan_Waktu_Siswa->Durasi_Soal($this->input->post('id_tes'))->row()->Durasi;
