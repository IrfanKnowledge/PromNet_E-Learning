...
class Tes_Ketuntasan_Jawaban_Siswa extends CI_Controller {
	...
	public function Ambil_Jawaban()
	{
		if($this->session->userdata('user') != 'siswa') {
			...
			$tes_ketuntasan_jawaban = $this->M_Tes_Ketuntasan_Jawaban->Ambil_Jawaban($this->input->post('id_setiap_soal'))->result();
			//$this->session->set_userdata('hasil', $tes_ketuntasan_jawaban);

			// $i = 1;
			// $hasil = '<script type="text/javascript">';
			// foreach ($tes_ketuntasan_jawaban as $record) {
			...
			// $hasil .= '</script>';
			// echo $hasil;
			echo json_encode($tes_ketuntasan_jawaban);
	...
	public function Submit()
	{
		if($this->session->userdata('user') != 'siswa') {
			...
			$this->load->model('M_Tes_Ketuntasan_Jawaban');
			$this->M_Tes_Ketuntasan_Jawaban->Submit($this->input->post('array_form'), $this->input->post('id_tes'));

			//redirect menampilkan terlebih dahulu halaman yang dituju kemudian berpindah halaman
			//redirect('Tes_Ketuntasan_Siswa/index/'. $this->uri->segment(3));

			$output = '<meta http-equiv="refresh" content="0; URL=http://localhost/PromNet_E-Learning/index.php/Tes_Ketuntasan_Siswa/index/';
