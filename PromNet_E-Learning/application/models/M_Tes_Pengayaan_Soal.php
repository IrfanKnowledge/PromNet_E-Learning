...
class M_Tes_Pengayaan_Soal extends CI_Model {
  ...
  //Berikut fungsi yang mengambil ID di Tabel Tes_Pengayaan
  //..berdasarkan id_Sesi yang kita peroleh dari Fungsi getByKdMapel_SesiKe
  //..Yang akan digunakan sebagai acuan di Tabel Tes_Pengayaan_Soal
  ...
  //Berikut fungsi yang menampilkan Tes Pengayaan siswa
  ...
  public function getWaktu_TesPengayaan($id_tes = -1)
  {
  ...
  {
    if ( empty($this->getWaktu_TesPengayaan($id_tes)->row()) ) {
      $status_waktu_mulai_tes = 0;
      $status_waktu_berakhir_tes = 0;
    }else{
      $status_waktu_mulai_tes = $this->Perbedaan_Waktu( $this->getWaktu_TesPengayaan($id_tes)->row()->Waktu_Mulai_Tes );
      $status_waktu_berakhir_tes = $this->Perbedaan_Waktu( $this->getWaktu_TesPengayaan($id_tes)->row()->Waktu_Berakhir_Tes );
    }

    /*-------------------Untuk Cek - Cek---------------------------------
    echo $this->getWaktu_TesPengayaan($id_tes)->row()->Waktu_Mulai_Tes;
    echo "<br>";
    echo $this->getWaktu_TesPengayaan($id_tes)->row()->Waktu_Berakhir_Tes;
    echo "<br>";
  ...
  public function getStatusPengerjaan($id_tes = -1)
  {
    ...
    $this->db->from('tes_pengayaan_waktu_siswa');
  ...
  public function Default_Jawaban($id_tes = -1)
  {
    ...
    $this->db->insert_batch('tes_pengayaan_jawaban', $data);
  }
  ...
  public function Default_WaktuMulai_dan_Jawaban($id_tes = -1)
  {
    ...
    $this->db->trans_start();
    $this->db->insert('tes_pengayaan_waktu_siswa', $data);
