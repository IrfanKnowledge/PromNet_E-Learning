...
class M_Tes_Pengayaan_Jawaban extends CI_Model {
  // public function getAll()
  // {
  //   $this->db->select('*');
  //   $this->db->from('tes_pengayaan_jawaban');
  //
  //   return  $this->db->get();
  // }
  //
  ...
  public function Ubah_Jawaban($jawaban = array())
  {
    if ($jawaban != array()) {
      ...
      $this->db->update_batch('tes_pengayaan_jawaban', $data_2, 'id_Soal');
    }
  }
  ...
  public function Ubah_Status_Pengerjaan($id_tes = -1)
  {
    ...
    $this->db->where('id_Tes', $id_tes);
    $this->db->update('tes_pengayaan_waktu_siswa');
  }
  ...
  public function Ubah_Nilai($data_jawaban = array(), $id_tes = -1)
  {
    ...
    $this->db->insert('tes_pengayaan_nilai', $data);
    ...
    // $this->db->where('NIS', $this->session->userdata('username'));
    // $this->db->where('id_Tes', $id_tes);
    // $this->db->update('tes_pengayaan_nilai');
  }
